...
A partir do \textit{pool de classificadores}, são selecionados os modelos mais promissores para a composição dos \textit{ensembles} heterogêneos, por meio da Seleção por Acurácia (SA) e Seleção por Acurácia e Diversidade (SAD). Gerando, assim, os \textit{ensembles} que foram criados a quantidade de modelos [2, 3, 4, 5], já que em \textcite{dietterich2000ensemble, kuncheva2004classifier} é falo que ensembles com poucos modelos performam muito proximo ou igual a que tem muitos modelos, além dos \textit{ensembles} com todos os modelos, que ficou chamado de Sem Seleção (SS). Esses \textit{ensembles} combinam as previsões dos modelos base, gerando decisões mais robustas. 
