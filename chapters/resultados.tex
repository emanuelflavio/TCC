...
\begin{table}[H]
	\centering
	\caption{Métricas dos Modelos Individuais(Letterboxd - Com Pré-Processamento	)}
	\resizebox{\textwidth}{!}{
...
Tais achados reforçam o que é estabelecido na literatura: \textit{modelos lineares são frequentemente superiores} quando se trata de classificação textual com TF-IDF. \cite{hassan2022analytics, galke2022we, kucuk2024comparative} Além disso, a análise evidenciou o impacto variável do pré-processamento em diferentes bases, podendo prejudicar, melhorar ou não ter efeito, dependendo da base.

\section{DESEMPENHO DOS ENSEMBLES SEM SELEÇÃO}

Uma das etapas dos experimentos foi a avaliação do \textit{ensemble} sem seleção, com o objetivo de observar o comportamento e como a diversidade de modelos impacta a performance do \textit{ensemble} tanto no \textit{Voting} quanto no \textit{Stacking}.
...
Na versão sem pré-processamento da Letterboxd, ambos os ensembles obtiveram desempenho semelhante, com F1-Scores entre \textit{0.80} e \textit{0.82}. O Stacking ainda apresentou um leve ganho na classe positiva, mas a diferença geral foi menor do que a observada no cenário com pré-processamento.
...
\begin{table}[H]
	\centering
	\caption{Acurácia dos Modelos Individuais (Letterboxd - Sem Pré-Processamento)}
	\begin{tabular}{lc}
		\toprule
...
\begin{table}[H]
	\centering
	\caption{Acurácia dos Modelos Individuais (Letterboxd - Com Pré-Processamento)}
	\begin{tabular}{lc}
		\toprule
		\textbf{Modelo} & \textbf{Acurácia} \\
		\midrule
...
Os valores de acurácia apresentados nesta seção são fundamentais para identificar os modelos com melhor desempenho em cada cenário e, assim, orientar a composição dos \textit{ensembles}, tanto pela estratégia de Seleção por Acurácia (SA) quanto pela Seleção por Acurácia e Diversidade (SAD). Foi observado que os modelos que tiveream um desempenho superior foram: Regressão Logística, SVM e SGD.

\subsection{DESEMPENHO DOS ENSEMBLES COM 2 MODELOS}

Nesta subseção, analisam-se os \textit{ensembles} compostos por dois modelos, selecionados pela estratégia de Seleção por Acurácia (SA), destacando o impacto da escolha dos modelos sobre o desempenho geral do ensemble.
...
Nesta subseção, analisam-se os \textit{ensembles} compostos por três modelos, selecionados pela estratégia de Seleção por Acurácia (SA), demonstrando o impacto da adição do terceiro modelo na performance geral do \textit{ensemble}.
...
Já nesta subseção, analisam-se os \textit{ensembles} compostos por quatro modelos, selecionados pela estratégia de Seleção por Acurácia (SA), demonstrando como a inclusão do quarto modelo influencia os resultados e o desempenho geral do \textit{ensemble}.
...
Por fim, nesta subseção, analisam-se os \textit{ensembles} compostos por cinco modelos, selecionados pela estratégia de Seleção por Acurácia (SA), demonstrando como a ampliação do \textit{ensemble} afeta sua capacidade de generalização e desempenho sobre o conjunto de dados.
...
Nesta subseção, apresentam-se os resultados obtidos pelos \textit{ensembles} compostos por dois modelos, construídos a partir da técnica de Seleção por Acurácia e Diversidade (SAD), nos quais são mostradas as métricas de desempenho por classe, permitindo identificar quais pares apresentam melhor desempenho no \textit{ensemble}.
...
Aqui, apresentam-se os resultados obtidos pelos \textit{ensembles} compostos por três modelos, construídos a partir da técnica de Seleção por Acurácia e Diversidade (SAD), mostrando o efeito da adição de um terceiro modelo sobre a performance geral do \textit{ensemble}.
...
Já nesta parte, apresentam-se os resultados obtidos pelos \textit{ensembles} compostos por quatro modelos, construídos a partir da técnica de Seleção por Acurácia e Diversidade (SAD), mostrando como a inserção de um quarto modelo impacta a eficácia. 
...
Por fim, nesta subseção, apresentam-se os resultados obtidos pelos \textit{ensembles} compostos por cinco modelos, construídos a partir da técnica de Seleção por Acurácia e Diversidade (SAD), analisando como a ampliação do ensemble com um quinto modelo impacta as métricas de desempenho.
...
Os resultados obtidos neste estudo corroboram e expandem as evidências encontradas na literatura correlata, demonstrando que o uso de ensembles heterogêneos é uma estratégia robusta para a análise de sentimentos. Assim como observado por \textcite{kazmaier2022power}, que alcançaram melhorias de até 5,53\% com a combinação de modelos, e por \textcite{kilimci2018deep}, cujos ensembles superaram classificadores individuais em todos os cenários, este trabalho confirmou que a diversidade entre modelos é um fator determinante para o ganho de desempenho, especialmente por meio da técnica de Stacking. Enquanto o estudo de \textcite{inbook} destacou a competitividade de representações simples como o TF-IDF, esta pesquisa reafirmou a supremacia de modelos lineares (SVM, Regressão Logística e SGD) quando associados a essa vetorização. Por fim, a introdução da base inédita do Letterboxd permitiu observar que a eficácia do pré-processamento é variável e dependente do domínio, reforçando a conclusão de \textcite{ribeiro2016sentibench} de que não existe um método universalmente superior para todas as bases de dados.
